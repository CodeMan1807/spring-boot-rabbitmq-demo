...
public class BroadCast {
    ...
    static final String ROUTING_KEY_NAME = "routing_key";
    ...
    public Binding mailBinding() {
        return BindingBuilder.bind(mailQueue()).to(mailExchange()).with(ROUTING_KEY_NAME);
    }
    ...
    public Queue ttlQueue(){
        ...
        Queue queue = new Queue("ttl.queue", true, false, false, arguments);
        return queue;
    }
...
//    }
    ...
    public Binding binding() {
        // 队列出问题进入私信队列，没有匹配到该交换器，进入备用交换器
        return BindingBuilder.bind(queue()).to(exchange()).with("product").noargs();
    }

    @Bean Binding bindingTtl(){
        return BindingBuilder.bind(ttlQueue()).to(exchange()).with("ttl").noargs();
    }
